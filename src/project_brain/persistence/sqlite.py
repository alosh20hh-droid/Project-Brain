...
class SQLiteProjectStore:
 ...
 def load_state(self,project_id:str)->tuple[dict[str,Any],int]:
  with self.connect() as db:
   row=db.execute("SELECT state_json,version FROM project_state WHERE project_id=?",(project_id,)).fetchone()
  return ({},0) if row is None else (json.loads(row[0]),int(row[1]))
 def put(self,namespace:str,key:str,value:Any)->None:
  raw=json.dumps(value,ensure_ascii=False,sort_keys=True)
  with self.connect() as db:
   db.execute("INSERT INTO kv(namespace,key,value_json) VALUES(?,?,?) ON CONFLICT(namespace,key) DO UPDATE SET value_json=excluded.value_json",(namespace,key,raw))
